...
class Ventas extends AuthController
{
    ...
    public function getTotalProductosVendidos()
    {
        try {
            header('Content-Type: application/json; charset=utf-8');
            $total = $this->model->getTotalProductosVendidos();
            echo json_encode(['total' => $total], JSON_UNESCAPED_UNICODE);
        } catch (Exception $e) {
            error_log("Error en getTotalProductosVendidos: " . $e->getMessage());
    ...
    public function getVentasDiarias()
    {
        try {
            header('Content-Type: application/json; charset=utf-8');
            $total = $this->model->getVentasDiarias();
            echo json_encode(['total' => $total], JSON_UNESCAPED_UNICODE);
        } catch (Exception $e) {
            error_log("Error en getVentasDiarias: " . $e->getMessage());
            echo json_encode(['total' => 0], JSON_UNESCAPED_UNICODE);
        }
        die();
    }
